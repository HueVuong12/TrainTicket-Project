...
public class QuanLyNhanVien_GUI extends JPanel implements ActionListener,MouseListener{
	...
	public QuanLyNhanVien_GUI() {
		...
		jp_timKiem.setBounds(300, 45, 1160, 75);
		add(jp_timKiem);
		...
		jp_thongTinNV.setBounds(10, 45, 280, 520);
		add(jp_thongTinNV);
		...
		scrollPane.setBounds(300, 153, 1160, 412);
		add(scrollPane);
		...
		if(o.equals(btnThem)) {
			if (validData()) {
				...
				if (nv != null) {
					...
					NhanVien existingNV = dsnv.getNhanVienByMaNV(nv.getMaNV());
					if (existingNV != null) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (row != -1) {
			NhanVien nv = dsnv.getNhanVienByMaNV(table_NV.getModel().getValueAt(row, 1).toString());
	...
	public void search() {
		if (validData()) {
			...
			NhanVien nv = dsnv.getNhanVienByMaNV(maNV);
			int stt = 1;
